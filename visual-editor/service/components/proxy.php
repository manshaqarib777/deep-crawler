...
define("PROXY_PREFIX", "https://scraper.site/visual-editor/service/components/proxy.php/");
...
if (strpos($url, "//") === 0) $url = "https:" . $url; //Assume that any supplied URLs starting with // are HTTP URLs.
if (!preg_match("@^.*://@", $url)) $url = "https://" . $url; //Assume that any supplied URLs without a scheme are HTTP URLs.

// recordLog($url);
if(strpos($url, 'bmo.c') > -1){
	echo 'Blocked URL';
	exit;
}
